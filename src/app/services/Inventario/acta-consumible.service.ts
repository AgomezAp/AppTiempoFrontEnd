...
export interface CrearActaConsumibleRequest {
  tipoInventarioCodigo: 'aseo' | 'papeleria' | 'botiquin' | 'desechables' | 'dotacion' | 'herramientas';
  nombreReceptor: string;
  cedulaReceptor?: string;
  cargoReceptor: string;
  areaReceptor?: string;
...
export class ActaConsumibleService {
  ...
  obtenerActas(tipoInventarioCodigo?: string, estado?: string): Observable<ActaConsumible[]> {
    ...
    return this.http.get<ActaConsumible[]>(`${this.apiUrl}/api/inventario/actas-consumibles`, { params });
  }

  /**
   * Obtener actas por tipo (aseo o papeleria)
   */
  obtenerActasPorTipo(codigo: 'aseo' | 'papeleria' | 'botiquin' | 'desechables' | 'dotacion' | 'herramientas', estado?: string): Observable<ActaConsumible[]> {
    let params = new HttpParams();
    if (estado) params = params.set('estado', estado);
    
    return this.http.get<ActaConsumible[]>(`${this.apiUrl}/api/inventario/actas-consumibles/tipo/${codigo}`, { params });
  }
  ...
  obtenerActaPorId(id: number): Observable<ActaConsumible> {
    return this.http.get<ActaConsumible>(`${this.apiUrl}/api/inventario/actas-consumibles/${id}`);
  }
  ...
  crearActa(request: CrearActaConsumibleRequest): Observable<{ msg: string; acta: ActaConsumible; token: string }> {
    return this.http.post<{ msg: string; acta: ActaConsumible; token: string }>(
      `${this.apiUrl}/api/inventario/actas-consumibles`,
      request
    );
  }
  ...
  obtenerActaPorToken(token: string): Observable<ActaConsumible> {
    return this.http.get<ActaConsumible>(`${this.apiUrl}/api/inventario/actas-consumibles/firma/${token}`);
  }
  ...
  firmarActa(token: string, firma: string, cedulaReceptor?: string): Observable<{ msg: string; acta: ActaConsumible }> {
    return this.http.post<{ msg: string; acta: ActaConsumible }>(
      `${this.apiUrl}/api/inventario/actas-consumibles/firma/${token}`,
  ...
  rechazarActa(token: string, motivoRechazo: string): Observable<{ msg: string; acta: ActaConsumible }> {
    return this.http.post<{ msg: string; acta: ActaConsumible }>(
      `${this.apiUrl}/api/inventario/actas-consumibles/rechazar/${token}`,
      { motivoRechazo }
    );
  }
  ...
  reenviarCorreo(id: number): Observable<{ msg: string }> {
    return this.http.post<{ msg: string }>(
      `${this.apiUrl}/api/inventario/actas-consumibles/${id}/reenviar`,
      {}
    );
  }
  ...
  obtenerEstadisticas(tipoInventarioCodigo?: string): Observable<EstadisticasActas> {
    ...
    return this.http.get<EstadisticasActas>(`${this.apiUrl}/api/inventario/actas-consumibles/estadisticas`, { params });
  }
  ...
  cancelarActa(id: number): Observable<{ msg: string }> {
    return this.http.request<{ msg: string }>(
      'DELETE',
      `${this.apiUrl}/api/inventario/actas-consumibles/${id}`,
  ...
  eliminarActa(id: number): Observable<{ msg: string }> {
    return this.http.delete<{ msg: string }>(
      `${this.apiUrl}/api/inventario/actas-consumibles/${id}/eliminar`
    );
  }
}
