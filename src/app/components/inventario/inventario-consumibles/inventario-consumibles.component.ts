...
import { Consumible, EstadisticasConsumibles, TipoInventario } from '../../../interfaces/Inventario/mobiliario-consumible';
import { debounceTime, distinctUntilChanged, Subject, takeUntil } from 'rxjs';
import { ConsumibleService } from '../../../services/Inventario/consumible.service';
import { MobiliarioService } from '../../../services/Inventario/mobiliario.service';
import { WebsocketService } from '../../../services/Inventario/websocket.service';
...
export class InventarioConsumiblesComponent implements OnInit, OnDestroy {
  ...
  get permiteSustraccionStock(): boolean {
    return this.tipoInventarioCodigo !== 'botiquin' && this.tipoInventarioCodigo !== 'papeleria';
  }
  /**
  ...
  abrirModalStock(
    ...
    if (
      ...
      this.error =
        `El ${this.tituloInventario.toLowerCase()} no permite salidas ni ajustes manuales. Use actas de entrega para retirar productos.`;
      this.cdr.markForCheck();
      return;
    }
  ...
  restarStockRapido(consumible: Consumible, event: Event): void {
    ...
    if (!this.permiteSustraccionStock) {
      this.error =
        `El ${this.tituloInventario.toLowerCase()} no permite retiros rápidos. Use actas de entrega para retirar productos.`;
      this.cdr.markForCheck();
      return;
    }
