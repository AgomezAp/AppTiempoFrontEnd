...
import { Dispositivo, EstadisticasInventario } from '../../../interfaces/Inventario/inventario';
...
export class InventarioComponent implements OnInit, OnDestroy {
  ...
  aplicarFiltros(): void {
    this.dispositivosFiltrados = this.dispositivos.filter((d) => {
      const cumpleEstado =
        this.filtroEstado === 'todos' || d.estado === this.filtroEstado;
      const cumpleCategoria =
        this.filtroCategoria === 'todas' ||
        d.categoria === this.filtroCategoria;
  ...
  get dispositivosPaginados(): Dispositivo[] {
    ...
    return this.dispositivosFiltrados.slice(
      inicio,
  ...
  eliminarDispositivo(dispositivo: Dispositivo): void {
    const cantidadInfo =
      dispositivo.tipoRegistro === 'stock'
    ...
    if (confirmar) {
      this.inventarioService
        .eliminarDispositivo(dispositivo.id!, 'Eliminación del inventario')
        .subscribe({
          next: (response) => {
            this.cargarDispositivos();
            this.cargarEstadisticas();
  ...
  restarStock(dispositivo: Dispositivo): void {
    const id = dispositivo.id!;
    if ((dispositivo.stockActual || 0) <= 0 || this.stockEnProceso.has(id))
      return;

    this.stockEnProceso.add(id);
    this.inventarioService
      .retirarStock(id, 1, 'Ajuste de inventario')
      .subscribe({
        next: (response) => {
          dispositivo.stockActual = response.stockActual;
          this.stockEnProceso.delete(id);
