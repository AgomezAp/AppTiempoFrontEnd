...
import { Mobiliario } from '../../../interfaces/Inventario/mobiliario-consumible';
import { MobiliarioService } from '../../../services/Inventario/mobiliario.service';
import { ActaMobiliarioService } from '../../../services/Inventario/acta-mobiliario.service';
...
@Component({
  ...
  styleUrls: ['./crear-acta-mobiliario.component.css'],
})
export class CrearActaMobiliarioComponent implements OnInit {
  ...
  filtrarMuebles(): void {
    ...
    this.mueblesFiltrados = this.mueblesDisponibles.filter(
      (m) =>
  ...
  agregarMueble(mueble: Mobiliario): void {
    if (this.mueblesSeleccionados.find((m) => m.mobiliario.id === mueble.id))
      return;
    ...
    this.mueblesDisponibles = this.mueblesDisponibles.filter(
      (m) => m.id !== mueble.id,
    );
    this.filtrarMuebles();
  }
  ...
  async crearActa(): Promise<void> {
    ...
    if (this.mueblesSeleccionados.length === 0) {
      this.errorMessage = 'Debe seleccionar al menos un mueble';
      return;
    }
