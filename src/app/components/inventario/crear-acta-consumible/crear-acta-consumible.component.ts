...
import { ActaConsumibleService, CrearActaConsumibleRequest } from '../../../services/Inventario/acta-consumible.service';
import { Consumible } from '../../../interfaces/Inventario/mobiliario-consumible';
...
@Component({
  ...
  styleUrls: ['./crear-acta-consumible.component.css'],
})
export class CrearActaConsumibleComponent implements OnInit {
  ...
  cargarArticulosDisponibles(): void {
    ...
    this.consumibleService.obtenerConsumiblesDisponibles().subscribe({
      next: (data) => {
        ...
        this.articulosDisponibles = data.filter(
          (c) =>
            c.tipoInventario?.codigo === this.tipoInventario &&
            c.stockActual > 0,
        );
      ...
      error: (err) => {
        ...
        this.consumibleService
          .obtenerConsumiblesPorTipo(this.tipoInventario)
          .subscribe({
            next: (data) => {
  ...
  filtrarArticulos(): void {
    ...
    this.articulosFiltrados = this.articulosDisponibles.filter(
      (a) =>
  ...
  agregarArticulo(consumible: Consumible): void {
    ...
    if (
      this.articulosSeleccionados.find((a) => a.consumible.id === consumible.id)
    ) {
      return;
    }
    ...
    this.articulosDisponibles = this.articulosDisponibles.filter(
      (a) => a.id !== consumible.id,
    );
