...
@Component({
  ...
  styleUrls: ['./firma-consumible.component.css'],
})
export class FirmaConsumibleComponent implements OnInit, AfterViewInit {
    ...
    this.http
      ...
      .subscribe({
        ...
        error: (err) => {
          this.firmando = false;
          alert(
            err.error?.msg || 'Error al procesar la firma. Intente nuevamente.',
          );
        },
      });
  }
  ...
  getTotalArticulos(): number {
    return (
      this.datosActa?.articulos.reduce((sum, a) => sum + a.cantidad, 0) || 0
    );
  }
