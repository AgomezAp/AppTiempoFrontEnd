...
import { InfoFirmaEppResponse } from '../../interfaces/ssgt';

@Component({
  selector: 'app-ssgt-epp-firmar',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './ssgt-epp-firmar.component.html',
  styleUrl: './ssgt-epp-firmar.component.css',
})
export class SsgtEppFirmarComponent implements OnInit, AfterViewInit {
  ...
  infoFirma: InfoFirmaEppResponse | null = null;
  ...
  cargarInfoFirma(): void {
    this.ssgtService.obtenerInfoFirmaEpp(this.token).subscribe({
      next: (data) => {
        this.infoFirma = data;
        this.loading = false;
        setTimeout(() => this.initCanvas(), 100);
  ...
  firmar(): void {
    ...
    this.ssgtService.firmarEntregaEpp(this.token, { firma: firmaBase64 }).subscribe({
      next: () => {
        this.success = true;
        this.firmando = false;
      },
